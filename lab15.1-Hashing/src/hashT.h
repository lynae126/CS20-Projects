...
+ hashT(int size)
+ ~hashT()
+ hashT<elemType>::print() const : void
+ remove(int hashIndex, const elemType& rec) : void
+ template <class elemType>
+ retrieve(int hashIndex, elemType& rec) const : void
+ isItemAtEqual(int hashIndex, const elemType& rec) const : bool
+ search(int& hashIndex, const elemType& rec, bool& found) const : void
+ template <class elemType>
+ insert(int hashIndex, const elemType& rec) : void
+ getLength() const : int
-----------------------------------
*/
#ifndef H_Htable
...
hashT<elemType>::hashT(int size)
{
    ...
    for (int i = 0; i < size; i++)
        ...
        cerr << "Table size is less than zero.";
}
